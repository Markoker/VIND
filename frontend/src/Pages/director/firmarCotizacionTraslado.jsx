...
export function FirmarCotizacionTrasladoDirector() {
    const {id} = useParams();
    ...
    const aprobarCotizacion = async () => {
        ...
        try {
            ...
            formData.append('tipo_item', 'traslado');
            await axios.post(`http://localhost:8000/solicitudes/${id}/${rut}/firmar-cotizacion/traslado`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
            alert("Cotización de traslado aprobada exitosamente");
    ...
    const confirmarRechazo = async () => {
        ...
        try {
            await axios.post(`http://localhost:8000/solicitudes/${id}/${rut}/rechazar-cotizacion-traslado-director`, null, {
                params: {comentario: comentarioRechazo}
            });
            setMostrarComentario(false);
            setComentarioRechazo("");
            alert("Cotización de traslado rechazada exitosamente");
            navigate("/director/solicitudes");
        } catch (err) {
    ...
    const descargarCotizacion = async (numeroCotizacion) => {
        try {
            const response = await axios.get(
                `http://localhost:8000/cotizacion/traslado/${detalle.cotizacion.traslado.id}/cotizacion/${numeroCotizacion}`,
                { responseType: 'blob' }
            );
            ...
            link.href = url;
            link.setAttribute('download', `cotizacion_${numeroCotizacion}_traslado_${detalle.cotizacion.traslado.id}.pdf`);
    ...
    return (
        <div style={{padding: "20px"}}>
            <h1>Firmar Cotización de Traslado - Solicitud #{detalle.id}</h1>
            ...
            {detalle.cotizacion && detalle.cotizacion.traslado && (
                <div style={{marginBottom: "20px"}}>
                    <h3>Cotización de Traslado para Firmar</h3>
                    <table border="1" style={{width: "100%", borderCollapse: "collapse"}}>
                        <tbody>
                            <tr>
                                <td style={{padding: "10px", fontWeight: "bold"}}>Proveedor</td>
                                <td style={{padding: "10px"}}>{detalle.cotizacion.traslado.nombre_proveedor}</td>
                                <td style={{padding: "10px", fontWeight: "bold"}}>RUT</td>
                                <td style={{padding: "10px"}}>{detalle.cotizacion.traslado.rut_proveedor}</td>
                            </tr>
                            <tr>
                                <td style={{padding: "10px", fontWeight: "bold"}}>Email</td>
                                <td style={{padding: "10px"}}>{detalle.cotizacion.traslado.correo_proveedor}</td>
                                <td style={{padding: "10px", fontWeight: "bold"}}>Monto</td>
                                <td style={{padding: "10px"}}>${detalle.cotizacion.traslado.monto}</td>
                            </tr>
                        </tbody>
                    </table>

                    {detalle.cotizacion.traslado.cotizaciones && detalle.cotizacion.traslado.cotizaciones.length > 0 && (
